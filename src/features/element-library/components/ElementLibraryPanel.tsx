...
import type {
  ...
  ElementLibraryItem,
  ElementLibraryItemType,
} from "../types";

type DragPayload =
  | {
      kind: "text";
      preset: NonNullable<ElementLibraryItem["textPreset"]>;
      label: string;
    }
  | {
      kind: "shape";
      preset: NonNullable<ElementLibraryItem["shapePreset"]>;
      label: string;
    }
  ...
  | {
      kind: "transition";
      preset: NonNullable<ElementLibraryItem["transitionPreset"]>;
      label: string;
    };
...
export function ElementLibraryPanel() {
  ...
  const { items, category, setCategory, query, setQuery, total } =
    useElementCatalog(uploadedItems);
          ...
          {total === 0 ? (
            ...
            <div className="grid grid-cols-2 gap-2 max-[1024px]:grid-cols-1">
              {items.map((item) => {
                ...
                const isImportedAudio =
                  item.type === "audio" && item.category === "audio";
                const isImportedImage =
                  item.type === "image" && item.category === "media";
                const isImportedVideo =
                  item.type === "video" && item.category === "media";
                ...
                return (
                  ...
                  >
                        ...
                        <div className="absolute inset-0 z-10 flex items-center justify-center bg-[#1a1a20]">
                          ...
                          {item.textPreset && (
                            <span
                              ...
                              style={{
                                fontSize:
                                  item.textPreset === "title"
                                    ? "1.2rem"
                                    : item.textPreset === "subtitle"
                                      ? "0.9rem"
                                      : "0.7rem",
                                fontWeight:
                                  item.textPreset === "title"
                          ...
                          {item.shapePreset && (
                            <div
                              className={
                                item.shapePreset === "rectangle"
                                  ? "h-6 w-10"
                                  : "h-10 w-10"
                              }
                              style={{
                                backgroundColor:
                                  item.shapePreset === "ellipse"
                                    ? "#22c55e"
                                    : "#4f46e5",
                                borderRadius:
                                  item.shapePreset === "ellipse"
                                    ? "9999px"
                                    : item.shapePreset === "background"
                                      ? "12px"
                                      : "8px",
                              }}
                            />
                        ...
                        <div
                          ...
                          onMouseDown={(
                            ...
                            const cleanup = (mouseUpEvent?: MouseEvent) => {
                              ...
                              const clientX =
                                mouseUpEvent?.clientX ?? lastPointerX;
                              const clientY =
                                mouseUpEvent?.clientY ?? lastPointerY;
                              ...
                              if (!withinPreview) {
                                console.log(
                                  "[ElementLibrary][drag] end outside preview - ignored",
                                  { clientX, clientY },
                                );
                                return;
                              }
                              ...
                              if (preview) {
                                ...
                                if (activePayload.kind === "text") {
                                  void addTextElement({
                                    ...
                                    console.error(
                                      "[ElementLibrary][drag] text create failed",
                                      error,
                                    );
                                    setFeedback(
                                      `No se pudo añadir el texto: ${getCreateTextErrorMessage(error)}`,
                              ...
                              window.dispatchEvent(
                                new CustomEvent<DragEndDetail>(
                                  "element-library:drag-end",
                                  { detail },
                                ),
                              );
                            };
                    ...
                    <div className="px-0.5 pt-1.5">
                      <p className="truncate text-[11px] text-[#e5e7eb]">
                        {item.name}
                      </p>
                      {item.description && (
          ...
          <>
            ...
            <input
              ...
              onChange={async (e) => {
                ...
                const generateAudioWaveform = (file: File): Promise<string> => {
                  return new Promise((resolve) => {
                    ...
                    for (let i = 0; i < bars; i++) {
                      const height =
                        20 + (Math.sin(seed + i * 0.5) * 0.5 + 0.5) * 50;
                      const x =
                        (canvas.width - bars * (barWidth + gap)) / 2 +
                        i * (barWidth + gap);
                ...
                console.log(
                  "[ElementLibrary] import files ->",
                  files.map((f) => ({ name: f.name, type: f.type })),
                );

                window.dispatchEvent(
                  new CustomEvent("element-library:import", {
                    detail: { files },
                  }),
                );
                e.target.value = "";
