...
function ProjectCard({
  ...
  const handleClick = () => {
    if (isMenuOpen || isEditingName || isLoading) return;
    void onLoad(project.id);
  };

  const commitRename = () => {
    const trimmed = editName.trim();
    if (trimmed && trimmed !== project.name) {
      void onRename(project.id, trimmed);
    }
      ...
      >
        <div className="flex items-center gap-4 p-4">
          ...
          <div className="min-w-0 flex-1">
            ...
            <div
              className={`mt-2 flex items-center gap-3 text-xs ${isDarkMode ? "text-[#6b7280]" : "text-[#64748b]"}`}
            >
          ...
          <button
            className="flex-shrink-0 rounded-md border border-[#4f46e5] bg-[#6366f1] px-4 py-2 text-xs font-semibold text-white transition hover:bg-[#818cf8] disabled:cursor-not-allowed disabled:opacity-60"
            disabled={isLoading}
            onClick={(event) => {
              event.stopPropagation();
              void onLoad(project.id);
  ...
  return (
    ...
    >
      ...
      <div className="space-y-3 p-4">
        ...
        >
          <span
            className={`truncate text-xs ${isDarkMode ? "text-[#6b7280]" : "text-[#64748b]"}`}
          >
            {project.owner} · {project.collaborators} colaboradores ·{" "}
            {project.resolution.w}x{project.resolution.h}
          </span>
          <button
            className="rounded-md border border-[#4f46e5] bg-[#6366f1] px-3 py-1.5 text-xs font-semibold text-white transition hover:bg-[#818cf8] disabled:cursor-not-allowed disabled:opacity-60"
            disabled={isLoading}
            onClick={(event) => {
              event.stopPropagation();
              void onLoad(project.id);
...
export function GalleryPage() {
  ...
  const apiEnabled = isProjectsApiEnabled();
  const userInitials =
    user?.name
      .split(" ")
      .map((part) => part[0])
  ...
  const [isLoadingProjects, setIsLoadingProjects] = useState(apiEnabled);
  ...
  const localGalleryProjects = useMemo(
  ...
  const allGalleryProjects = useMemo(
    () => (apiEnabled ? projects : localGalleryProjects),
    [apiEnabled, localGalleryProjects, projects],
  );

  const managedProjectIds = useMemo(
    () =>
      new Set(
        apiEnabled ? projects.map((project) => project.id) : presentationProjects.map((project) => project.id),
      ),
    [apiEnabled, projects],
  );
  ...
  const visibleProjects = useMemo(() => {
    ...
    return allGalleryProjects.filter((project) => {
      ...
      const matchesSubject =
        !activeSubjectFilter ||
        project.tags.some(
          (tag) => tag.toLowerCase() === activeSubjectFilter.toLowerCase(),
        );
      if (!matchesSubject) {
        return false;
      }
  ...
  const handleLoadProject = async (projectId: string) => {
    ...
    try {
      if (apiEnabled) {
        ...
        navigate(
          projectWasLoaded ? `/editor?project=${encodeURIComponent(projectId)}` : "/editor",
        );
      }
    } catch (error) {
      setLoadError(
        error instanceof Error ? error.message : "No se pudo cargar este proyecto.",
      );
    } finally {
  ...
  const handleRenameProject = async (projectId: string, newName: string) => {
    ...
    if (apiEnabled) {
      setLoadError(null);
      try {
        const updatedProject = await updateProject(projectId, { name: trimmedName });
          ...
          currentProjects.map((project) =>
            project.id === projectId
              ? {
                  ...
                  lastEditedAt: updatedProject.updatedAt || project.lastEditedAt,
                }
              : project,
          ),
        );
        ...
        setLoadError(
          error instanceof Error ? error.message : "No se pudo renombrar este proyecto.",
        );
      }
      return;
    }

    if (renamePresentationProject(projectId, trimmedName)) {
  ...
  return (
    <main
      className={`flex h-screen flex-col overflow-hidden px-4 py-7 sm:px-6 lg:px-10 ${
        isDarkMode
          ? "bg-[#0d0d11] text-[#f0f0f4]"
          : "bg-[#f3f5fb] text-[#111827]"
      }`}
    >
      <header className="relative mb-4 flex flex-wrap items-center justify-between gap-4 md:flex-nowrap">
        ...
        <div className="pointer-events-none absolute left-[19rem] top-1/2 hidden -translate-y-1/2 lg:block">
          <h2 className="text-2xl font-semibold leading-none sm:text-3xl">
      ...
      <section className="flex min-h-0 flex-1 w-full gap-4">
        ...
        >
            ...
            <div className="mt-2 space-y-1.5">
              ...
              <button
                className={`flex w-full items-center gap-2 rounded-md border border-transparent bg-transparent px-3 py-2 text-left text-sm transition ${
                  isDarkMode
                    ? "text-[#b3b9c7] hover:bg-[#1a1a24] hover:text-white"
          ...
          <div
            className={`mt-auto border-t pt-4 ${isDarkMode ? "border-[#2a2a34]" : "border-[#d5dbe8]"}`}
          >
            <p
              className={`px-2 text-[11px] uppercase tracking-wide ${isDarkMode ? "text-[#8b93a7]" : "text-[#64748b]"}`}
            >
              ...
              <div
                className={`mt-2 flex items-center justify-between rounded-lg border p-2 ${
                  isDarkMode
                    ...
                    : "border-[#d5dbe8] bg-[#f8f9fd]"
                }`}
              >
                <Moon
                  className={`h-4 w-4 ${isDarkMode ? "text-[#c7d2fe]" : "text-[#94a3b8]"}`}
                />
                <button
                ...
                <Sun
                  className={`h-4 w-4 ${isDarkMode ? "text-[#94a3b8]" : "text-[#f59e0b]"}`}
                />
              </div>
            ) : null}
          </div>
        ...
        <section className="flex min-w-0 flex-1 flex-col">
          <section className="mb-5 grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
            ...
            >
              <p
                className={`text-xs uppercase tracking-wide ${isDarkMode ? "text-[#9ca3af]" : "text-[#64748b]"}`}
              >
                Total proyectos
              </p>
              <p
                className={`mt-1 text-2xl font-semibold ${isDarkMode ? "text-[#f9fafb]" : "text-[#0f172a]"}`}
              >
                {totalProjects}
              </p>
            </article>
          ...
          >
            ...
            <div
              className={`flex items-center gap-1 rounded-md border p-1 ${isDarkMode ? "border-[#2a2a34] bg-[#0f0f14]" : "border-[#d5dbe8] bg-[#f8f9fd]"}`}
            >
              <button
          ...
          {loadError ? (
            <div
              className={`mb-5 rounded-lg border p-4 text-sm ${
                isDarkMode
                  ? "border-[#7f1d1d] bg-[#2a1114] text-[#fecaca]"
          ...
          {isLoadingProjects ? (
            <div
              className={`rounded-lg border border-dashed p-10 text-center ${
                isDarkMode
                  ? "border-[#35353f] bg-[#15151b]"
                  : "border-[#c7d2e5] bg-white"
              }`}
            >
              ...
              >
                Estamos trayendo la biblioteca desde la base de datos.
              </p>
            </div>
            ...
            >
              <p className="text-base font-medium">
                {hasActiveSearchOrFilter
                  ...
                  : "No hay proyectos en la biblioteca"}
              </p>
              <p
                className={`mt-1 text-sm ${isDarkMode ? "text-[#9ca3af]" : "text-[#64748b]"}`}
              >
                {hasActiveSearchOrFilter
                  ...
                  : apiEnabled
                    ...
                    : "Crea o abre un proyecto para empezar a trabajar."}
              </p>
            </div>
          )}
        ...
        <div className="absolute inset-0 z-30 flex items-center justify-center bg-black/45 px-4">
          ...
          >
            ...
            <div className="mt-4 space-y-3">
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(event) =>
                    setNewProjectSubject(event.target.value)
                  }
            ...
            <div className="mt-5 flex justify-end gap-2">
              ...
              <button
                className="rounded-md border border-[#4f46e5] bg-[#6366f1] px-3 py-2 text-sm font-medium text-white transition hover:bg-[#818cf8] disabled:cursor-not-allowed disabled:opacity-60"
                disabled={isCreatingProject}
                onClick={() => void handleCreateNewProject()}
                type="button"
              >
                {isCreatingProject ? "Creando..." : "Crear y abrir"}
              </button>
            </div>
          </div>
        </div>
