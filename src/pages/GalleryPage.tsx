import { CalendarDays, Clock3, Grid3X3, LayoutList, PanelsTopLeft, Play } from 'lucide-react'
...
function isElementActiveAtTime(element: EditorElement, time: number): element is VisualFrameElement {
  if (element.type === 'audio' || element.type === 'transition') {
    return false
  }

  return element.startTime <= time && element.startTime + element.duration > time
}

function getFrameElementsAtTime(project: PresentationProject, time: number): FrameElementContext[] {
...
function getSlideTimes(project: PresentationProject): number[] {
  ...
  return [...times].sort((a, b) => a - b).filter((t) => t >= 0 && t < project.durationSeconds)
}
...
function useAnimatedPreview(project: PresentationProject, isHovered: boolean) {
  ...
  const slideDuration = 800 // ms per slide
  const pauseBeforeLoop = 600 // ms pause at end before looping
  ...
  useEffect(() => {
    ...
    const totalCycleDuration = slideTimes.length * slideDuration + pauseBeforeLoop
    ...
    const animate = (timestamp: number) => {
      ...
      const nextTime = slideTimes[slideIndex]
...
function ProjectCard({ project, viewMode, onLoad, formatDuration, formatDate }: ProjectCardProps) {
      ...
      <div className="pointer-events-none absolute inset-0 z-[2] flex flex-col justify-between p-3">
        <div className="flex items-start justify-between">
          <div className="flex gap-1">
            {isHovered && (
              <span className="inline-flex items-center gap-1 rounded-full bg-[#6366f1]/80 px-2 py-1 text-[10px] font-medium text-white backdrop-blur-[2px] animate-pulse">
      ...
      >
        <div className="flex items-center gap-4 p-4">
          ...
          <div className="flex-1 min-w-0">
            <h2 className="text-base font-semibold leading-tight truncate">{project.name}</h2>
            <p className="mt-1 text-sm text-[#9ca3af] line-clamp-2">{project.description}</p>
            <div className="mt-2 flex items-center gap-3 text-xs text-[#6b7280]">
              <span>{project.owner}</span>
              <span>·</span>
              <span>{project.collaborators} colaboradores</span>
              <span>·</span>
              <span>{project.resolution.w}x{project.resolution.h}</span>
            </div>
          ...
          <button
            className="rounded-md border border-[#4f46e5] bg-[#6366f1] px-4 py-2 text-xs font-semibold text-white transition hover:bg-[#818cf8] flex-shrink-0"
            onClick={(event) => {
